...
path_image_input_vti = os.path.join(output_dir, 'input_image_undef.vti')
input_image_from_mesh_vti = create_image_from_vtp(input_vtp,
                                                  spacing=[0.2, 0.2, 0.2],
                                                  bounds=input_vtp.GetBounds(),
...
coord_array, value_array = get_coord_value_array_for_image(input_image_undef, flat=False)

import scipy.io as sio
sio.savemat(os.path.join(output_dir, 'image_coords_values.mat'), {'coordinates':coord_array.astype(np.float32),
